...
def default_split_fn(r):
    import re
    # match name between slash and _<digits> at the end of the string
    # (slash in the beginning or _<digits> in the end or either may be missing)
    ...
    for (isplit, sk) in enumerate(sorted(sk2r.keys())):
        ...
        if average_group:
            for group in sorted(groups):
                ...
                l, = axarr[idx_row][idx_col].plot(usex, ymean, color=color, linestyle=linestyle)
                g2l[group] = l
                if shaded_err:
        ...
        if any(g2l.keys()):
            ax.legend(
                g2l.values(),
                ['%s (%i)'%(compact_legend(g), g2c[g]) for g in g2l] if average_group else g2l.keys(),
