...
basedir = [
    f'log_ppo/{env}.ppo2.clip_lr_decay.worker_1.norm_obs.norm_rew.model_type_clip.left_cliprange_0.8.right_cliprange_1.2-',
    f'log_ppo_cd/{env}.ppo2.clip_lr_decay.worker_1.norm_obs.norm_rew.model_type_clip.left_cliprange_0.8.right_cliprange_1.2.use_cd_True.sd_delta_1.0-'
]
NUM = len(basedir)
algo = algo.upper()
#label = ['ESPO', 'SD-ESPO', 'SD-ESPO-unbias(1/M,M)', 'SD-ESPO-value', 'SD-ESPO-rminus', 'SD-ESPO(no abs)-rminus', 'RSD-ESPO', 'LSD-ESPO']
label = [algo, f'SD-{algo}', f'SD-{algo}-unbias(1/M,M)', f'SD-{algo}-value', f'SD-{algo}-rminus', f'SD-{algo}(no abs)-rminus', f'RSD-{algo}', f'LSD-{algo}']
label = label[:NUM]
...
if exptype == 'one_side':
    method_list = [f'SD-{algo}', f'RSD-{algo}', f'LSD-{algo}']
elif exptype == f'sd_ppo':
    method_list = [algo, f'SD-{algo}']
